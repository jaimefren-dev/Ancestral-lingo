...
import { Heart, Zap, Award, CheckCircle, XCircle, Volume2, Lock, RotateCcw, Target, X, Music, Trophy, Star, BookOpen } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (gameState.view === 'lesson' && questions.length > 0 && !isLessonComplete) {
      const sessionData = {
        ...
        questions,
        currentIndex,
        matchedPairs, // Save matching game progress
        selectedOption, // Save selected answer
        lessonStatus // Save if it was correct/incorrect
      };
      localStorage.setItem(SESSION_STORAGE_KEY, JSON.stringify(sessionData));
    }
  }, [gameState, questions, currentIndex, isLessonComplete, matchedPairs, selectedOption, lessonStatus]);

  // --- Audio Services ---
  const playText = useCallback((text: string) => {
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      // Reset state ensures animation restarts if clicked repeatedly
      setIsSpeaking(false);
      
      let textToSpeak = text;
      
      // Ajustes fonéticos simples para mejorar la pronunciación con el motor TTS en Español
      if (gameState.activeLanguage === 'shuar') {
        textToSpeak = textToSpeak
          .replace(/j/gi, 'h') // La 'j' Shuar es suave (/h/), no fuerte (/x/) como en España.
          .replace(/ts/gi, 'tz') // Enfatizar la africada
          .replace(/k/gi, 'c') 
          .replace(/w/gi, 'gu') // Aproximación para 'wa' -> 'gua'
          .replace(/ü/gi, 'u')
          .replace(/ï/gi, 'i');
      }

      const utterance = new SpeechSynthesisUtterance(textToSpeak);
      // Usar español latinoamericano ('es-MX' o 'es-US') es MEJOR para lenguas andinas/amazónicas
      // que el español de España ('es-ES') debido a la suavidad de las consonantes.
      utterance.lang = 'es-MX'; 
      utterance.rate = 0.8; // Un poco más lento para claridad didáctica
  ...
  }, [gameState.activeLanguage]);

  // Effect to auto-play audio when a listening question appears
  useEffect(() => {
    if (gameState.view === 'lesson' && questions.length > 0 && lessonStatus === 'idle') {
  ...
  }, [gameState.view, currentIndex, questions, playText, lessonStatus]);
    ...
    if (savedSessionRaw) {
      try {
        ...
        if (
          ...
          // Restore exact state including selection and status
          setLessonStatus(savedSession.lessonStatus || 'idle');
          setSelectedOption(savedSession.selectedOption || null);
  ...
  const renderGoalsModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-white w-full max-w-lg rounded-[2rem] shadow-2xl flex flex-col max-h-[85vh] overflow-hidden animate-in slide-in-from-bottom-8 zoom-in-95 duration-300">
          ...
          <div className="p-6 border-b-2 border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
             <div className="flex items-center gap-3">
               ...
               <h2 className="text-2xl font-extrabold text-slate-700 tracking-tight">Metas y Logros</h2>
             </div>
             <button 
          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-6 bg-slate-50">
             {ACHIEVEMENTS.map((achievement) => {
               ...
               const percent = Math.min(100, (currentValue / target) * 100);
               const Icon = achievement.icon;

               // Color Styles based on type
               let iconColors = "bg-slate-200 text-slate-500";
               ...
               if (achievement.type === 'streak') {
                 ...
                 iconColors = "bg-yellow-100 text-yellow-500";
                 barColor = "bg-yellow-500";
               } else if (achievement.type === 'lessons') {
                 iconColors = "bg-blue-100 text-blue-500";
                 barColor = "bg-blue-500";
               }

               if (isClaimed) {
                 iconColors = "bg-yellow-400 text-white"; // Gold for claimed
               }
               ...
               return (
                 <div 
                    key={achievement.id} 
                    className={`
                      relative p-4 rounded-3xl border-2 border-b-4 bg-white transition-all
                      ${isClaimed 
                        ? 'border-yellow-400 bg-yellow-50' 
                        : 'border-slate-200'
                      }
                      ${isCompleted && !isClaimed ? 'border-sky-400 ring-2 ring-sky-200 ring-offset-2' : ''}
                    `}
                 >
                    <div className="flex gap-4">
                      {/* Icon */}
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 ${iconColors} transition-colors duration-500`}>
                      ...
                      <div className="flex-1 flex flex-col justify-center gap-2">
                        <div>
                          <h3 className="font-bold text-slate-700 leading-tight">{achievement.title}</h3>
                        ...
                        {!isClaimed && (
                          <div className="w-full h-4 bg-slate-100 rounded-full overflow-hidden relative">
                             <div 
                                ...
                                style={{ width: `${percent}%` }}
                             ></div>
                             <div className="absolute inset-0 flex items-center justify-center text-[10px] font-black text-slate-400 mix-blend-multiply">
                               {currentValue} / {target}
                             </div>
                          </div>
                        )}
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 flex justify-end">
                      {isClaimed ? (
                        <div className="flex items-center gap-2 text-yellow-600 font-bold text-sm bg-yellow-100 px-4 py-2 rounded-xl">
                        ...
                        <button 
                          ...
                          className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-b from-sky-400 to-sky-500 border-b-4 border-sky-700 text-white font-extrabold rounded-xl uppercase tracking-wider text-sm active:border-b-0 active:translate-y-1 transition-all animate-bounce shadow-lg shadow-sky-200 hover:brightness-110"
                        >
                          <Award className="w-5 h-5" />
                          RECLAMAR +{achievement.xpReward} XP
                        </button>
                      ) : (
  ...
  const renderHome = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center">
        ...
        <div className="w-full max-w-4xl p-4 flex gap-2">
           <button 
        ...
        {/* World Map / Categories - Grid Layout */}
        <main className="w-full max-w-4xl flex-1 p-6 grid grid-cols-2 md:grid-cols-3 gap-8 pb-20 justify-items-center content-start">
            ...
            return (
              <div key={cat.id} className="flex flex-col items-center relative w-full group">
                <button
                  ...
                  disabled={isLocked}
                  className={`
                    w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center text-4xl shadow-lg border-b-4 transition-transform active:translate-y-1 relative z-10
                ...
                <div className="mt-3 text-center">
                  <h3 className="font-bold text-slate-700 text-lg leading-tight">{cat.title}</h3>
  ...
  const renderLesson = () => {
    ...
    const AudioIcon = gameState.activeLanguage === 'shuar' ? Music : Volume2;

    return (
      <div className="min-h-[100dvh] md:min-h-screen bg-white md:bg-gray-100 flex flex-col items-center justify-center">
        <div className="w-full md:max-w-lg md:mx-auto md:bg-white md:shadow-xl md:rounded-3xl md:overflow-hidden md:h-[90vh] flex flex-col relative h-[100dvh]">
          {/* Top Bar */}
          ...
          {/* Question Area - Evenly spaced */}
          <div key={question.id} className="flex-1 w-full p-4 md:p-6 flex flex-col justify-evenly gap-6 max-w-md mx-auto overflow-y-auto">
               ...
               <div className="flex flex-col items-center justify-center gap-4 py-2 w-full">
                 {question.imageUrl ? (
                    <button
                      ...
                      className={`relative group rounded-3xl overflow-hidden shadow-lg border-4 border-slate-100 max-h-56 w-auto transition-transform active:scale-95 ${isSpeaking ? 'ring-4 ring-sky-300' : ''}`}
                    >
                      <img 
                      ...
                      <div className="absolute inset-0 bg-black/10 group-hover:bg-black/20 flex items-center justify-center transition-colors">
                         <div className={`${activeTheme.primary} p-4 rounded-full text-white shadow-xl`}>
                    ...
                    <button 
                        onClick={() => playText(question.audioText || '')}
                        className={`
                          ${activeTheme.primary} w-32 h-32 rounded-2xl flex items-center justify-center shadow-lg border-b-4 ${activeTheme.primaryBorder} active:translate-y-1 transition-all
                          ${isSpeaking ? 'animate-pulse scale-105 ring-4 ring-offset-2 ring-current' : ''}
                        `}
                    >
            ...
            {/* Type: Text Translation (Flashcard) */}
            {(question.type === 'translate_to_spanish') && (
              <div className="flex flex-col gap-6 items-center w-full">
                {/* Word Card */}
                <div className="flex items-center gap-4 p-4 border-2 border-slate-200 rounded-3xl w-full shadow-sm bg-white">
                  <button 
                    onClick={() => playText(question.audioText || '')}
                    className={`
                      p-4 rounded-2xl transition-all flex-shrink-0
                      ${isSpeaking ? 'bg-sky-100 text-sky-600 scale-110' : `bg-slate-100 ${activeTheme.text} hover:bg-slate-200`}
                    `}
                  >
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                      {gameState.activeLanguage === 'kichwa' ? 'Kichwa' : 'Shuar'}
                    </span>
                    <span className="text-2xl font-extrabold text-slate-700">{question.question}</span>
                  </div>
                </div>

                {/* Image below word - Uniform Distribution */}
                {question.imageUrl && (
                   <div className="w-full flex justify-center animate-in zoom-in-90 duration-500">
                      <img 
                        src={question.imageUrl} 
                        alt="Visual Context" 
                        className="rounded-3xl border-4 border-slate-100 w-full max-h-52 object-cover shadow-lg" 
                      />
                   </div>
                )}
              </div>
                    ...
                    <OptionCard 
                      ...
                      onClick={() => {
                         if(lessonStatus === 'idle') {
                           ...
                           // Play audio on selection if it matches one of the options (optional UX choice, kept simple here)
                           // playText(opt); 
                         }
                      }}
                      disabled={lessonStatus !== 'idle'}
                      shortcut={idx + 1}
                      onPlayAudio={() => playText(opt)}
                      AudioIcon={AudioIcon}
                    />
                  );
                })}
                ...
                {question.pairs.map((card) => {
                  ...
                  return (
                    <div
                      ...
                      className={`
                        h-24 rounded-2xl border-2 border-b-4 font-bold text-base transition-all cursor-pointer
                        flex items-center justify-center p-2 text-center leading-tight relative group select-none
                        ${isSelected 
                    ...
                    >
                      ...
                      {/* Absolute positioned audio button for cleaner layout in grid */}
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          playText(card.text);
                        }}
                        className="absolute top-1 right-1 p-1.5 rounded-lg hover:bg-black/5 text-current opacity-40 hover:opacity-100 transition-all"
